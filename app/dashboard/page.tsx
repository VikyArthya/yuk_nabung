...
// Helper function untuk mengonversi angka ke terbilang bahasa Indonesia
  ...
  function convertLessThanOneThousand(num: number): string {
    ...
    if (num < 10) {
      ...
      if (hundred === 1 && remainder === 0) {
        ...
        return 'seratus ' + convertLessThanOneThousand(remainder);
      } else {
        return units[hundred] + ' ratus' + (remainder > 0 ? ' ' + convertLessThanOneThousand(remainder) : '');
      }
    }
  }

  if (amount < 1000) {
    return convertLessThanOneThousand(amount);
  }

  let result = '';
  let divisor = 1000000000; // 1 miliar
  const scale = ['miliar', 'juta', 'ribu', ''];

  for (let i = 0; i < scale.length; i++) {
    if (amount >= divisor) {
      const portion = Math.floor(amount / divisor);
      const remainder = amount % divisor;

      if (scale[i] === 'ribu' && portion === 1) {
        result += 'seribu';
      } else {
        result += convertLessThanOneThousand(portion) + ' ' + scale[i];
      }
...
async function getDashboardData(userId: string) {
  // Check if user has any wallets first
...
export default async function DashboardPage() {
  ...
  // If user has no budget, show welcome screen with wallet validation
      ...
      <div className="bg-yellow-200 min-h-screen">
        ...
        <main className="max-w-4xl mx-auto py-6 sm:py-8 px-4 sm:px-6 lg:px-8">
          <div className="text-center space-y-8">
            {/* Show different content based on wallet existence */}
            {!dashboardData.hasWallets ? (
              /* No Wallets Screen */
              <Card className="neo-card-raised">
                <CardHeader className="pb-6 sm:pb-8 neo-red border-b-4 border-black">
                  <div className="space-y-4">
                    <div className="text-6xl">💳</div>
                    <div>
                      <h2 className="neo-heading text-xl sm:text-2xl text-white">
                        Buat Dompet Terlebih Dahulu
                      </h2>
                      <p className="neo-text mt-2 text-white">
                ...
                <CardContent className="pt-0 sm:pt-0">
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white border-2 border-black shadow-[4px_4px_0px_black] p-6 text-center neo-interactive hover:shadow-[6px_6px_0px_black] hover:-translate-y-0.5 hover:-translate-x-0.5">
                        <div className="text-3xl mb-3">💳</div>
                        <h3 className="font-black mb-2">
                          Kelola Banyak Dompet
                        </h3>
                        <p className="neo-text text-sm">
                          Bank, E-Wallet, atau cash - kelola semuanya di satu tempat
                        </p>
                      </div>
                      ...
                      <div className="bg-white border-2 border-black shadow-[4px_4px_0px_black] p-6 text-center neo-interactive hover:shadow-[6px_6px_0px_black] hover:-translate-y-0.5 hover:-translate-x-0.5">
                        ...
                        <p className="neo-text text-sm">
                          Alokasikan budget ke berbagai dompet untuk kontrol yang lebih baik
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border-2 border-black shadow-[4px_4px_0px_black] p-6 text-center neo-interactive hover:shadow-[6px_6px_0px_black] hover:-translate-y-0.5 hover:-translate-x-0.5">
                      ...
                      <p className="neo-text text-sm">
                        Buat dompet untuk mengelola keuangan Anda sebelum membuat budget
                      </p>
                    </div>
                    ...
                    {/* Tips */}
                    <div className="bg-white border-2 border-black shadow-[4px_4px_0px_black] neo-interactive hover:shadow-[6px_6px_0px_black] hover:-translate-y-0.5 hover:-translate-x-0.5">
                      <div className="p-4 border-b-4 border-black">
                        <h4 className="font-black">💡 Tips Dompet</h4>
                      </div>
                      <div className="p-4 pt-0">
                        <ul className="neo-text text-sm space-y-2 text-left font-bold">
                          <li>• Buat dompet untuk setiap sumber dana (bank, e-wallet, cash)</li>
                          <li>• Beri nama yang jelas untuk setiap dompet</li>
                          <li>• Setelah memiliki dompet, Anda bisa membuat budget bulanan</li>
                          <li>• Alokasikan budget ke dompet yang berbeda untuk kontrol yang lebih baik</li>
                        </ul>
                      </div>
                    </div>
                  </div>
              ...
              /* Has Wallets but No Budget Screen */
              <Card className="neo-card-raised">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <h2 className="neo-heading text-xl sm:text-2xl">
                        Waktunya Membuat Budget!
                      </h2>
                      <p className="neo-text mt-2">
                        Anda sudah memiliki {dashboardData.wallets.length} dompet. Sekarang buat budget bulanan
                      </p>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="pt-0 sm:pt-0">
                  <div className="space-y-6">
                    {/* Wallet Summary */}
                    <div className="bg-green-100 border-2 border-black shadow-[4px_4px_0px_black] p-6">
                      <h3 className="font-black mb-4 text-green-800">Dompet Anda:</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {dashboardData.wallets.map((wallet) => (
                          <div key={wallet.id} className="bg-white border-2 border-black shadow-[2px_2px_0px_black] p-4">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-3">
                                <div className="w-10 h-10 bg-green-100 border-2 border-black shadow-[1px_1px_0px_black] flex items-center justify-center">
                                  <span className="text-lg font-black">💳</span>
                                </div>
                                <div className="text-left">
                                  <h4 className="font-black">{wallet.name}</h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white border-2 border-black shadow-[4px_4px_0px_black] p-6 text-center neo-interactive hover:shadow-[6px_6px_0px_black] hover:-translate-y-0.5 hover:-translate-x-0.5">
                        ...
                        <p className="neo-text text-sm">
                          Daily budget otomatis dihitung dari weekly budget yang Anda buat
                        </p>
                      </div>
                    ...
                    <div className="bg-white border-2 border-black shadow-[4px_4px_0px_black] p-6 text-center neo-interactive hover:shadow-[6px_6px_0px_black] hover:-translate-y-0.5 hover:-translate-x-0.5">
                      ...
                      <p className="neo-text text-sm">
                        Buat budget bulanan dan alokasikan ke dompet Anda
                      </p>
                    </div>
                    ...
                    <div className="bg-white border-2 border-black shadow-[4px_4px_0px_black] neo-interactive hover:shadow-[6px_6px_0px_black] hover:-translate-y-0.5 hover:-translate-x-0.5">
                      <div className="p-4 border-b-4 border-black">
                        <h4 className="font-black">💡 Tips Budget</h4>
                      </div>
                      <div className="p-4 pt-0">
                        <ul className="neo-text text-sm space-y-2 text-left font-bold">
                          <li>• Buat budget bulanan dengan gaji dan target pengeluaran</li>
                          <li>• Daily budget otomatis dihitung: weekly budget ÷ 7 hari</li>
                          <li>• Alokasikan budget ke dompet yang sudah Anda buat</li>
                          <li>• Catat setiap pengeluaran harian untuk tracking yang akurat</li>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
          ...
          <Card className="md:col-span-1 neo-card-raised">
            <CardHeader className="pb-3 sm:pb-6 neo-green">
              ...
              <CardDescription className="text-xs sm:text-sm neo-text text-white">
                Akumulasi dari sisa budget bulan ini
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-0 sm:pt-0">
              ...
              <p className="text-xs sm:text-sm neo-text mt-1 first-letter:uppercase">
                {numberToWords(dashboardData.user.savingsBalance)} rupiah
              </p>
            </CardContent>
          </Card>
        </div>
